...
class Resolve extends \Magento\Backend\App\Action
{
   ...
   protected $messageManager;
			...
			if ($savedObj->getRecordId()) {
		        ...
		        if (!count($orderProblems)) {
                    ...
                    $problemModel->setTabletStatus("Resolved");
          			...
          			$previousOrderStatus = $this->getPreviousOrderStatus($order_no);
					if($previousOrderStatus){
						$this->messageManager->addSuccess("Problem resolved Sucessfully, Order Back to Queue");
			    	}else{
				    	$this->messageManager->addSuccess("Problem resolved Sucessfully, Order not Back to Queue");
				    }
		        }else{
		        	$this->messageManager->addSuccess("Problem resolved Sucessfully, Order not Back to Queue");
		        }
	...
	public function getPreviousOrderStatus($orderNumber)
	{
	    $secondToLastOrderStatus = false;
		// Load order by increment ID
		$order = $this->orderFactory->create()->loadByIncrementId($orderNumber);

		if ($order->getId()) {
			$previousStatuses = $order->getStatusHistories();
			$count = count($previousStatuses);
			if ($count >= 2) {
				// Loop through the status histories and retrieve the second-to-last status
				$i = 0;
				foreach ($previousStatuses as $status) {
					if ($i == $count - ($count - 1)) {
						$secondToLastOrderStatus = true;
						$order->setStatus($status->getStatus());
						$order->addStatusToHistory($status->getStatus(), 'Problem resolved Sucessfully from Dashboard, Order Back to Queue');
						$order->save();
						break;
					}
					$i++;
		...
		return $secondToLastOrderStatus;
	}
}